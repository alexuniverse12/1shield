...
export class OneInchQuote {
  private static getOneInchTokenAddress = (erc20: RecipeERC20Info) => {
  ...
  static supportsNetwork = (networkName: NetworkName) => {
    try {
      oneInchApiSubdomain(networkName);
      return true;
    } catch {
      return false;
    }
  ...
  static getSwapQuote: GetSwapQuote = async ({
    ...
    const sellTokenAddress = this.getOneInchTokenAddress(sellERC20Amount);
    const buyTokenAddress = this.getOneInchTokenAddress(buyERC20Info);
    ...
    try {
      ...
      } = await getOneInchData<ZeroXAPIPriceData>(
          OneInchApiEndpoint.GetSwapQuote,
        networkName,
        params,
      );
