...
describe('FORK-one-inch-swap-recipe', function run() {
  this.timeout(120000);
  ...
  it('[FORK] Should run one-inch-swap-recipe', async function run() {
    if (shouldSkipForkTest(networkName)) {
      this.skip();
      return;
    }
    ...
    const expectedSpender =
      OneInchQuote.zeroXExchangeProxyContractAddress(networkName);
    expect(quote.spender).to.equal(
      expectedSpender,
      '1 inch contract does not match.',
    );
