...
import { OneInchSwapRecipe } from '../one-inch-swap-recipe';
...
import { OneInchQuote } from '../../../api/one-inch';
...
describe('one-inch-swap-recipe', () => {
  before(() => {
    ...
    stub0xQuote = Sinon.stub(OneInchQuote, 'getSwapQuote').resolves(quote);
  ...
  it('Should create one-inch-swap-recipe with amount', async () => {
    const recipe = new OneInchSwapRecipe(sellToken, buyToken, slippagePercentage);
  ...
  it('Should test one-inch-swap-recipe error cases', async () => {
    const recipe = new OneInchSwapRecipe(sellToken, buyToken, slippagePercentage);
