...
import { OneInchSwapStep } from '../one-inch-swap-step';
...
import { OneInchConfig } from '../../../../models/one-inch-config';
...
describe('one-inch-swap-step', () => {
  before(() => {
    OneInchConfig.PROXY_API_DOMAIN = undefined;
  });

  it('Should create one-inch-swap step with spender', async () => {
    const step = new OneInchSwapStep(quote, sellToken);
  ...
  it('Should create one-inch-swap step without spender', async () => {
    const step = new OneInchSwapStep({ ...quote, spender: undefined }, sellToken);
    ...
    expect(output.name).to.equal('0x Exchange Swap');
    expect(output.description).to.equal(
      'Swaps two ERC20 tokens using 1 inch DEX Aggregator.',
    );
  ...
  it('Should test one-inch-swap step error cases', async () => {
    const step = new OneInchSwapStep(quote, sellToken);
    ...
    await expect(
      ...
      '1 inch Exchange Swap step is invalid. No step inputs match filter.',
    );
    ...
    await expect(
      ...
      '1 inch Exchange Swap step is invalid. Specified amount 10000 exceeds balance 2000.',
    );
  });
});
